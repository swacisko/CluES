...
//        cerr.rdbuf(nullptr);
    ...
    const bool ADD_SIGTERM_CHECK = false;
    if(!ADD_SIGTERM_CHECK){
        Global::max_runtime_in_seconds = 600;
...
//        if( E > 300'000 && avg_deg > 20){ // #TEST - removing triangle check from localSearch if graph is 'dense'
	...
	/*{
		clog << "******************************************CAUTION! Enabling all swaps!" << endl << endl; 
		//cnf.max_recursion_depth = 5;
		
		cnf.neg_use_triangle_swaps = true;
		cnf.neg_use_join_clusters = true;  // original false
		cnf.neg_use_node_interchange = true; cnf.neg_node_interchanging_frequency = 10; // original false
		cnf.neg_use_chain2_swaps = true; cnf.neg_chain2_swaps_frequency = 13; // original false

		cnf.neg_move_frequency = 2;

		cnf.neg_use_edge_swaps = true;
		cnf.neg_edge_swaps_frequency = 7;

		cnf.neg_use_triangle_swaps = true;
		cnf.neg_triangle_swaps_frequency = 17; // originally 70
