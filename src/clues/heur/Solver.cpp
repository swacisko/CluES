...
#include <clues/heur/SwapCandidates/ComponentExpansionAttraction.h>
#include <clues/heur/SwapCandidates/ComponentExpansionRepulsion.h>
#include <clues/heur/SwapCandidates/SwpCndNode.h>
#include <clues/heur/SwapCandidates/SwpCndEdge.h>
#include <clues/heur/SwapCandidates/SwpCndTriangle.h>
#include <clues/heur/SwapCandidates/SwpCndEO.h>
...
//    if( recurrence_depth == 0 ){
        assert( counters != nullptr && "Must assign counters object to solver before calling run_fast()" );
        (*counters)["rd_res_before_#" + to_string(recurrence_depth)] = best_result;
        (*counters)["rd_time_before_#" + to_string(recurrence_depth)] = Global::secondsFromStart() * 1000;
//    }

    createClusterGraph();
    known_solutions.clear(); known_clg_partitions.clear();

    (*counters)["rd_clg_N_#" + to_string(recurrence_depth)] = clg.N;
    (*counters)["rd_clg_E_#" + to_string(recurrence_depth)] = accumulate( ALL(clg.V), 0, [](auto s, auto & v){
        return s + v.size();
    } ) / 2;
    ...
    if( recurrence_depth < cnf->max_recursion_depth ){ // running recursively

        int REPS = cnf->granularity_frequency; // original 3
        ...
        solver.counters = counters;
        solver.lower_level_best_partition_to_induce = best_partition;

        solver.best_result = best_result; // #TEST - this may cause some assertions to fail...
        solver.run_fast();
        ...
        // if we do not recurse any more, than we set partition to simply single nodes of cluster graph clg
        // this should solve the bug... (hopefully)
        partition = VI( clg.N );
        iota(ALL(partition),0);
    }

    bool repeat_improvement = true;

    while(repeat_improvement) {
        repeat_improvement = false;

        if(!Global::disable_all_logs) {
            clog << endl;
            logSpacing(recurrence_depth);
            clog << "Starting repeated improvements, partition.size(): " << partition.size() << endl;
        }
        ...
        if (!Global::checkTle()) { // refinement of found solution
            ...
            if (improve_using_ls) {
                ...
                auto res_before = best_result;

                auto [part_oV, part_clg] = localSearch();
                compareToBestSolutionAndUpdate(part_oV);

                if( best_result < res_before && recurrence_depth == 0 ){
                    repeat_improvement = true;

                    if (!Global::disable_all_logs) {
                        clog << endl << endl;
                        logSpacing(recurrence_depth);
                        clog << "REAPEATING IMPROVEMENT!, res_before: " << res_before << " < " << best_result
                             << " = best_result" << endl;
                    }
                }
    ...
    (*counters)["rd_res_after_#" + to_string(recurrence_depth)] = best_result;
    (*counters)["rd_time_after_#" + to_string(recurrence_depth)] = Global::secondsFromStart() * 1000;
